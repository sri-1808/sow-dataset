...
class MTEvalToExcel:
    ...
    def create_excel_file(self, conversation_data: List[Dict], filename: str = "mt_eval_dataset.xlsx"):
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            dialogue_df.to_excel(writer, sheet_name='Dialogue_Summary', index=False)
                    ...
                    # Complete conversations sheet (readable format)
            conversations_readable = self.create_readable_conversations(conversation_data)
            conversations_df = pd.DataFrame(conversations_readable)
            conversations_df.to_excel(writer, sheet_name='Complete_Conversations', index=False)
    ...
    def create_readable_conversations(self, conversation_data: List[Dict]) -> List[Dict]:
        """Create readable conversation format showing full dialogues"""
        ...
        # Create readable format for each dialogue
        for dialogue_id, turns in dialogue_groups.items():
            ...
            conversation_text = []
            conversation_text.append(f"=== DIALOGUE: {dialogue_id} ===")
            conversation_text.append(f"Task: {turns[0]['task_type']}")
            conversation_text.append(f"Total Turns: {len(turns)}")
            conversation_text.append("")
            
            # Add each turn
            total_scores = {metric: [] for metric in self.evaluation_metrics.keys()}
            
            for turn in turns:
                conversation_text.append(f"--- TURN {turn['turn_number']} ---")
                conversation_text.append(f"USER: {turn['user_message']}")
                conversation_text.append("")
                conversation_text.append(f"GROUND TRUTH RESPONSE: {turn['ground_truth_response']}")
                conversation_text.append("")
                
                # Add evaluation scores for this turn
                if any(f'{metric}_score' in turn for metric in self.evaluation_metrics.keys()):
                    conversation_text.append("EVALUATION SCORES:")
                    for metric in self.evaluation_metrics.keys():
                        if f'{metric}_score' in turn:
                            score = turn[f'{metric}_score']
                            total_scores[metric].append(score)
                            conversation_text.append(f"  • {metric.title()}: {score}/5.0")
                    if 'average_score' in turn:
                        conversation_text.append(f"  • Average: {turn['average_score']}/5.0")
                conversation_text.append("")
                conversation_text.append("=" * 50)
                conversation_text.append("")
            
            # Calculate dialogue averages
            dialogue_averages = {}
            for metric, scores in total_scores.items():
                if scores:
                    dialogue_averages[f'avg_{metric}'] = round(sum(scores) / len(scores), 2)
            
            overall_avg = sum(dialogue_averages.values()) / len(dialogue_averages) if dialogue_averages else 0
            
            # Add dialogue summary
            conversation_text.append("=== DIALOGUE SUMMARY ===")
            conversation_text.append(f"Average Scores:")
            for metric, avg_score in dialogue_averages.items():
                clean_metric = metric.replace('avg_', '').title()
                conversation_text.append(f"  • {clean_metric}: {avg_score}/5.0")
            conversation_text.append(f"  • Overall Average: {round(overall_avg, 2)}/5.0")
            
            # Create record for Excel
            readable_conversations.append({
                'dialogue_id': dialogue_id,
                'task_type': turns[0]['task_type'],
                'task_name': turns[0]['task_name'],
                'total_turns': len(turns),
                'conversation_length_category': turns[0].get('conversation_length_category', 'N/A'),
                'task_difficulty': turns[0].get('task_difficulty', 'N/A'),
                'complete_conversation': self.clean_text_for_excel('\n'.join(conversation_text)),
                'dialogue_avg_faithfulness': dialogue_averages.get('avg_faithfulness', 0),
                'dialogue_avg_completeness': dialogue_averages.get('avg_completeness', 0),
                'dialogue_avg_naturalness': dialogue_averages.get('avg_naturalness', 0),
                'dialogue_avg_appropriateness': dialogue_averages.get('avg_appropriateness', 0),
                'dialogue_avg_relevance': dialogue_averages.get('avg_relevance', 0),
                'dialogue_avg_coherence': dialogue_averages.get('avg_coherence', 0),
                'dialogue_avg_helpfulness': dialogue_averages.get('avg_helpfulness', 0),
                'dialogue_overall_avg': round(overall_avg, 2),
                'first_user_message': self.clean_text_for_excel(turns[0]['user_message']),
                'first_ground_truth': self.clean_text_for_excel(turns[0]['ground_truth_response']),
                'last_user_message': self.clean_text_for_excel(turns[-1]['user_message']),
                'last_ground_truth': self.clean_text_for_excel(turns[-1]['ground_truth_response'])
            })
        
        return readable_conversations
...
if __name__ == "__main__":
    ...
    converter.run("mt_eval_multiturn_with_evaluation_metrics_new.xlsx")